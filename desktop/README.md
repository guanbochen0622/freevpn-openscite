...
ChatGPT 帳號頁新增「改用裝置碼登入」、官方網址備援、取消登入及登入失敗原因。主模型可在閱讀區或 AI 模型設定選擇。刪除重複的 OpenAI 服務選項。

Gemini／Claude 預設為官方網站手動往返：登入官方網站、貼上準備好的問題及圖片、貼回完整回答；這不是自動帳號授權。API 自動分析留在選用進階設定。混合統整也會等待手動步驟完成。
