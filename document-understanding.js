...
$('documentToolsToggle').onclick=()=>{const hidden=$('documentTools').classList.toggle('hidden');$('documentToolsToggle').setAttribute('aria-expanded',String(!hidden));if(!hidden)showOcr(current());};
...
async function snapshot(pageNo,pdf=state.reader.pdf){
 ...
 const canvas=document.createElement('canvas');canvas.width=Math.ceil(viewport.width);canvas.height=Math.ceil(viewport.height);const task=page.render({canvasContext:canvas.getContext('2d',{alpha:false}),viewport});await task.promise;return canvas;
}
function cancel(){if(!job)return;job.cancelled=true;job.worker?.terminate();job.reject?.(new Error('已取消辨識'));status(tr('已取消辨識'));}
function reset(){epoch++;cancel();ocrPages=new Map();structures=new Map();evidenceLinks=[];lastAnswer=null;$('ocrReview').classList.add('hidden');$('structureResults').replaceChildren();status('');}
function guard(j){if(j.cancelled||j.epoch!==epoch||j.token!==state.reader.renderToken)throw Error('已取消辨識');}
async function cache(pageNo,entry){try{await idbPut('ocr-v1:'+readerKey()+':'+pageNo,entry);}catch{status(tr('辨識已完成，但無法儲存快取'));}}
function adopt(pageNo,entry,text){entry.adopted=true;entry.corrected=text;ocrPages.set(pageNo,entry);state.reader.pageTexts[pageNo-1]=text;rebuildIndex();cache(pageNo,entry);attachOcr(pageNo);}
...
async function runOcr(){
 ...
 showTools();const pageNo=current(),token=state.reader.renderToken,lang=$('ocrLanguage').value;
 ...
 try{
  ...
  const entry={...result,native:ocrPages.get(pageNo)?.native??state.reader.pageTexts[pageNo-1]??'',language:lang,adopted:false,corrected:result.text};ocrPages.set(pageNo,entry);
  if(entry.native.trim().length<30)adopt(pageNo,entry,result.text);else await cache(pageNo,entry);
  showOcr(pageNo);status(tr('OCR 完成，請核對草稿')+' · '+tr(`第 ${pageNo} 頁`));
...
function showOcr(pageNo){const entry=ocrPages.get(pageNo);if(Number($('ocrReview').dataset.page)!==pageNo){const old=ocrPages.get(Number($('ocrReview').dataset.page));if(old)old.corrected=$('ocrTranscript').value;}$('ocrReview').classList.toggle('hidden',!entry);$('ocrReview').dataset.page=String(pageNo);if(entry)$('ocrTranscript').value=entry.corrected;}
...
$('restoreNativeText').onclick=()=>{const n=Number($('ocrReview').dataset.page),entry=ocrPages.get(n);if(entry){entry.adopted=false;state.reader.pageTexts[n-1]=entry.native;rebuildIndex();cache(n,entry);const wrap=document.querySelector(`.pdf-page[data-page="${n}"]`);wrap?.querySelector('.ocr-text-layer')?.remove();const layer=wrap?.querySelector('.textLayer');if(layer)layer.style.display='';status(tr('已還原原始文字層'));}};
...
async function renderStructures(value,pageNo){
 ...
 if(token!==state.reader.renderToken||structures.get(pageNo)!==value)return;
 ...
 for(const f of value.formulas){const card=document.createElement('section'),math=document.createElement('div');card.className='structure-card';math.className='formula-display';try{katex.render(f.latex,math,{displayMode:true,throwOnError:true,trust:false,strict:'error',maxExpand:200,maxSize:10,output:'htmlAndMathml'});}catch{math.textContent=tr('公式無法排版，請核對原頁');}card.append(math);const meaning=document.createElement('p');meaning.textContent=f.meaning;card.append(meaning);if(f.uncertainty){const warning=document.createElement('p');warning.className='structure-warning';warning.textContent=f.uncertainty;card.append(warning);}target.append(card);}
...
async function analyzeStructure(){
 if(!state.reader.pdf){toast('請先載入 PDF');return;}if(state.aiBusy||state.reader.figureBusy)return;
 const n=current(),token=state.reader.renderToken,pdf=state.reader.pdf;showTools();$('structurePage').disabled=true;status(tr('正在解析原頁公式與表格…'));
 try{const canvas=await snapshot(n,pdf);if(token!==state.reader.renderToken)return;const image=canvas.toDataURL('image/png');canvas.width=canvas.height=0;
 ...
 if(token!==state.reader.renderToken)return;const value=DocumentCore.validateStructure(raw);structures.set(n,value);await renderStructures(value,n);status(tr('解析完成，請核對原頁'));
 }catch(e){if(token===state.reader.renderToken)status(tr('解析未完成')+'：'+aiErrorMessage(e));}finally{$('structurePage').disabled=false;}
}
$('structurePage').onclick=analyzeStructure;
$('structureResults').onclick=e=>{const b=e.target.closest('[data-export-table]');if(!b)return;const table=structures.get(Number(b.dataset.tablePage))?.tables[Number(b.dataset.exportTable)];if(!table)return;const url=URL.createObjectURL(new Blob([DocumentCore.csv(table)],{type:'text/csv;charset=utf-8'})),a=document.createElement('a');a.href=url;a.download=`table-page-${b.dataset.tablePage}-${Number(b.dataset.exportTable)+1}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
...
async function groundedAnswer(question,selected='',previous=[]){
 ...
 const token=state.reader.renderToken,pageNo=selected?(state.reader.selectionPage||current()):current(),evidence=DocumentCore.segments(state.reader.pageTexts,question+' '+selected,pageNo);
 ...
 if(token!==state.reader.renderToken)throw Error('文件已變更');const claims=DocumentCore.validateAnswer(raw,evidence);lastAnswer={token,claims};return claims.map(c=>c.text+(c.sources.length?' '+[...new Set(c.sources.map(s=>`[Page ${s.page}]`))].join(' '):'')).join('\n\n');
...
document.addEventListener('research:current-page',()=>{if(!$('documentTools').classList.contains('hidden'))showOcr(current());});
document.addEventListener('research:page',e=>{attachOcr(Number(e.detail));if($('documentTools').classList.contains('hidden'))return;showOcr(current());});
document.addEventListener('research:document',async()=>{reset();const token=state.reader.renderToken,key=readerKey();for(let n=1;n<=state.reader.pages;n++){const entry=await idbGet('ocr-v1:'+key+':'+n);if(token!==state.reader.renderToken)return;if(!ocrPages.has(n)&&entry?.lines&&typeof entry.native==='string'){ocrPages.set(n,entry);if(entry.adopted){state.reader.pageTexts[n-1]=entry.corrected;attachOcr(n);}}}rebuildIndex();if(state.reader.fullText.replace(/\[Page \d+\]/g,'').trim().length<30){showTools();status(tr('這份文件缺少文字層，請執行 OCR 目前頁'));}});
window.DocumentUnderstanding={reset,runOcr,cancel,snapshot,groundedAnswer,appendEvidence,analyzeStructure,ocrPages:()=>ocrPages};
})();
