...
(async()=>{
 ...
 await page.evaluate(()=>sessionStorage.setItem(STORE.aiKey,'test-key-not-a-real-credential'));await page.fill('#askInput','What is the wavelength?');await page.click('#askBtn');await page.waitForFunction(()=>!state.aiBusy);console.log('AI answer diagnostic',await page.locator('#assistantOutput').textContent(),await page.evaluate(()=>({pages:state.reader.pages,errors:window.pdfEngineError})));await page.waitForSelector('.page-citation');await page.locator('.page-citation').click();assert.equal(await page.evaluate(()=>state.reader.currentPage),12);
