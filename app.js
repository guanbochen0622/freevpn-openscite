...
function goPdfPage(n){ n=Math.max(1,Math.min(state.reader.pages||1,Number(n)||1)); state.reader.currentPage=n; document.dispatchEvent(new CustomEvent('research:current-page',{detail:n})); $('pageJump').value=n; state.reader.paintPage?.(n); const el=currentPageElement();if(el){const vp=$('pdfViewport');vp.scrollTo({top:vp.scrollTop+el.getBoundingClientRect().top-vp.getBoundingClientRect().top-16,behavior:'instant'});} }
